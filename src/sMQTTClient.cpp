...
bool sMQTTClientWebSocket::sendFrame(WSopcode_t opcode, uint8_t * payload, size_t length, bool fin, bool headerToPayload)
{
    ...
    if(headerToPayload) {
        ...
        write((const char*)&payloadPtr[(WEBSOCKETS_MAX_HEADER_SIZE - headerSize)], (length + headerSize));
    } else {
        // send header
        write((const char*)&buffer[0], headerSize);

        if(payloadPtr && length > 0) {
            // send payload
            write((const char*)&payloadPtr[0], length);
        }
    }
